...
bool spell_ice_bolt( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
    ...
    if ( obj==NULL )
    {
       ...
       send_to_char( "A bolt of ice flies from your hand!", ch );
    }
    else
    {
    ...
    send_to_char( "You are struck by the ice bolt!!", victim );
...
/* check to see if victim is already flaming 
  ...
  shield->percent = 30 + get_psuedo_level( ch ) / 9;
  shield->hits = 8000 + get_psuedo_level( ch ) * 20;
  shield->sn	= sn;
  if ( char_login )
    shield->hits /= ( number_range( 2, 10 ) );
...
bool spell_lava_burst( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
   ...
   damage( ch, (CHAR_DATA *) vo, number_range( get_psuedo_level( ch ) * 4, get_psuedo_level( ch ) * 8 )  * save_mod, sn );
...
bool spell_heat_armor( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
   CHAR_DATA *victim = (CHAR_DATA *)vo;
    ...
    if ( saves_spell( level, victim ) && ( number_range( 0, 99 ) < 70 ) )
      return TRUE;
    {
      OBJ_DATA * heated_item = NULL;
      OBJ_DATA * prev_carried = NULL;
...
void do_stance( CHAR_DATA *ch, char *argument )
{
  ...
  if ( arg[0] == '\0' )
  {
    ...
    sprintf( cat_buf, "%s", "Type stance <stance name> to change your current fighting stance.\n\r You may place your current Stance in your prompt with a \%s\n\r" );
    safe_strcat( MSL, msg_buf, cat_buf );
