...
char *get_diplo_name( short value )
{
  ...
  if ( value < -450 )			name = "@@R  WAR  @@N";
  else if ( value < -300 )		name = "@@e HATRED@@N";
  else if ( value < -150 )		name = "@@dDISLIKE@@N";
  else if ( value < 150 )		name = "@@WNEUTRAL@@N";
  else if ( value < 300 )		name = "@@aRESPECT@@N";
  else if ( value < 450 )		name = "@@l TRUST @@N";
  else					name = "@@B ALLY  @@N";
...
void do_negotiate( CHAR_DATA *ch, char *argument )
{
   ...
   if ( !str_prefix( arg2, "peace" ) )
   {
     if  ( politics_data.diplomacy[ch->pcdata->clan][target_clan] < -450 )
     {
       ...
       {
         sprintf( buf, "@@NYou are currently at @@RWAR@@N with %s. Both clans must negotiate an end to the war first.\n\r", 
                       clan_table[target_clan].clan_name);
         send_to_char( buf, ch );
         return;
       }
...
void do_banish( CHAR_DATA *ch, char *argument )
{
   ...
   sprintf( buf, "%s has banished %s from clan %s.", ch->name, 
      victim->name, clan_table[ch->pcdata->clan].clan_name );
   monitor_chan( buf, MONITOR_CLAN );
    ...
    else
    {
      if ( ( victim = get_char_world( ch, arg2 ) ) != NULL ) 
      {
        ...
        else
        {
          ...
          send_to_char( buf, ch );
          sprintf( buf, "You have been @@ROUTCAST@@N from the @@eKindred@@N by the %s, and are now a @@dRenegade@@N!!!!!\n\r",
