    ...
    /* MAG Mod2: Convert Colour strings, while copying. Also expand buffer as neede. */
    {
     ...
     for ( ; count > 0; )
     {
      ...
      else
      {
       ...
       if (ch != NULL && !IS_SET(ch->act, PLR_COLOUR))
        continue;
...
void show_menu_to( DESCRIPTOR_DATA *d )
{
   ...
   sprintf( buf, "        2. Set Race.         Currently:%s\n\r",
      !IS_SET( d->check, CHECK_RACE ) ? "Not Set." :
   ...
   if ( IS_SET( d->check, CHECK_STATS ) )
      ...
      ch->pcdata->max_str, ch->pcdata->max_int, ch->pcdata->max_wis,
      ch->pcdata->max_dex, ch->pcdata->max_con );
   else
      sprintf( buf, "Not Set.\n\r" );
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
	...
	case 'n': case 'N': ch->sex = SEX_NEUTRAL; ch->login_sex = SEX_NEUTRAL;break;
	default:
	...
	if ( ch->level == 0 )
	{
	    ...
	    {
	    case APPLY_STR: ch->pcdata->max_str ++; break;
	    case APPLY_INT: ch->pcdata->max_int ++; break;
	    case APPLY_WIS: ch->pcdata->max_wis ++; break;
	    case APPLY_DEX: ch->pcdata->max_dex ++; break;
	    case APPLY_CON: ch->pcdata->max_con ++; break;
	    }
...
void do_hotreboot (CHAR_DATA *ch, char * argument)
{
	...
	/* We need to save all changed areas here....but HOW?!? */
