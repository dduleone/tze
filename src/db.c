...

    {
      ...
      for (index = SUPER_NONE; index < MAX_SUPER; index++ )
      {
        ...
        case SUPER_NONE : sprintf( buf, "%s", "ORDINARIES" );
	 			 break;
        case SUPER_VAMP : sprintf( buf, "%s", "@@rJUSTICARS@@N" );
                          break;
...
/*      letter = fread_letter( marksfp );
      if ( letter == '*' )
      {
	fread_to_eol( marksfp );
	continue;
      }

      if ( letter != '#' )
      {
        monitor_chan( "Load_marks: # not found.",MONITOR_BAD );
        break;
      }
*/
...
void reset_area( AREA_DATA *pArea )
{
	...
	case 'O':
	    ...
	    obj_to_room( obj, pRoomIndex );
          ...
          if (  ( IS_SET( pRoomIndex->room_flags, ROOM_NO_REPOP ) )
             && ( pRoomIndex->first_person != NULL )  )
          {
            extract_obj( obj );
          }
...
#endif
/*      
     if (load_char_obj(NULL,buf)==TRUE)
     {
      loaded_mob_addr->next        = char_list;
      char_list       = loaded_mob_addr;
      pMobIndex->count++;
      check_free_mem();
      return loaded_mob_addr;
     }  */
    }
...
void free_char( CHAR_DATA *ch )
{
	...
	free_string( ch->pcdata->title          );
        ...
        free_string( ch->pcdata->lastlogin );
	...
	{
	   free_string( ch->pcdata->ignore_list[cnt] );  
	}
...
void *_getmem(int size, const char *caller, int log)
{
  ...
  if (log&&mem_log)
    log_printf("getmem(%d)=%p from %s", size, mem, caller);
...
void *getmem(int size)
{
  ...
  {
    fprintf(stderr, "Out of memory; size %d requested.\n", size);
    raise(SIGSEGV);
  }
  nAllocPerm++;
  sAllocPerm += size;
...
void dispose(void *mem, int size)
{
  if ( !mem )
    return;
  nAllocPerm--;
...
void do_areas( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( msg, "@@R%d Areas listed.\n\r@@N Type areas all to list the entire "
              mudnamecolor " realm.\n\r@@N", foo );
    safe_strcat( MAX_STRING_LENGTH, buf, msg );
    send_to_char( buf, ch );
    return;
...
void do_memory( CHAR_DATA *ch, char *argument )
{
	...
	log_printf("SSM: %s called defrag_heap.", ch->name);
	defrag_heap();
	return;
    }
	...
	log_printf("%s turned off memory logging", ch->name);
	return;
      }
      else
      {
	...
	send_to_char("Memory logging is now ON.. remember to turn it off!\n\r",
		     ch);
	log_printf("%s turned on memory logging", ch->name);
	return;
      }
    }
