...
int defrag_heap()
{
  ...
  if (merges)
    log_printf("SSM: defrag_heap: made %d block merges.", merges);
  else
    log_printf("SSM: defrag_heap: resulted in 0 merges.");
...
RETRY:
  ...
  if (!ptr)
  {
    ...
    {
      int merges;
      log_printf("SSM: Attempting to optimize shared string heap.");
      merges = defrag_heap();
