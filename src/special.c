...
bool spec_mino_guard( CHAR_DATA *ch )
{
	...
	if (  ( ( pass = get_eq_char( victim, WEAR_HOLD ) ) != NULL )
           && ( pass->pIndexData->vnum == 6621 )  )
        {
...
/*       break;
    ...
    act( "$n hops on down the bunny trail.", ch, NULL, NULL, TO_ROOM );
    char_from_room( ch );
    
    char_to_room( ch, room );
    act( "$n hops into the room, carrying a huge basket of candy!", ch, NULL, NULL, TO_ROOM );
    ...
    act( "$n says 'Happy Easter $N!", ch, NULL, victim, TO_ROOM );

    act( "$n smiles at $N, and hands $M an easter egg, and a chocolate human.", ch, NULL, victim, TO_NOTVICT );
    act( "$N smiles at you, and hands you an easter egg and a chocolate human.", victim, NULL, ch, TO_CHAR );
    
    act( "$n eats both greedily, and feels MUCH better!.", victim, NULL, ch, TO_NOTVICT );
    act( "You eat both like a little kid, stuffing your mouth, and feel MUCH better!", victim, NULL, ch, TO_CHAR );
    sn = skill_lookup( "cure light" );
    spell_heal( sn, ch->level, ch, victim, NULL );
    return TRUE;
}
