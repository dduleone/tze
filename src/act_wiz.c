...
void do_rstat( CHAR_DATA *ch, char *argument )
{
	...
	location->vnum,
	location->light, 
	rev_table_lookup( tab_sector_types, location->sector_type ) );
    strcat( buf1, buf );
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC( victim ) )
    {
       ...
       victim->lvl[0],victim->lvl[1],victim->lvl[2],victim->lvl[3],
       victim->lvl[4] );
       strcat( buf1, buf );
       ...
       sprintf( buf, "Class Order: %s %s %s %s %s\n\r",
          ...
          class_table[victim->pcdata->order[3]].who_name, 
          class_table[victim->pcdata->order[4]].who_name );
       strcat( buf1, buf );
    ...
    if ( !IS_NPC( victim ) )
    {
	  ...
	  victim->pcdata->clan,
	  clan_table[victim->pcdata->clan].clan_abbr,
	  victim->balance );
       strcat( buf1, buf );
    }
...
void do_ofindlev( CHAR_DATA *ch, char *argument )
{
	...
	if ( ( pObjIndex = get_obj_index( vnum ) ) != NULL )
	{
      ...
      {
		sprintf( buf, "(REMORTAL) [%5d] %s\n\r",
		    pObjIndex->vnum, capitalize( pObjIndex->short_descr ) );
		safe_strcat(MAX_STRING_LENGTH, buf1, buf);
      } else {
		sprintf( buf, "(  MORTAL) [%5d] %s\n\r",
		    pObjIndex->vnum, capitalize( pObjIndex->short_descr ) );
		safe_strcat(MAX_STRING_LENGTH, buf1, buf);
      }
	    }
...
void do_users( CHAR_DATA *ch, char *argument )
{
	...
	if ( d->character != NULL )
	{
	    ...
	    switch ( d->connected )
	    {
		  ...
		  sprintf( buf3, "%s", "Get New Class   "); break;
	       case CON_GET_RACE:            
    ...
    strcat( buf, buf2 );
    sprintf( buf2, "%s%s%s", colour_string( ch, "stats" ), buf,
...
/*       || ( ( vamp ) && ( value < -1 || value > MAX_VAMP_LEVEL ) ) */  )
    ...
    if ( value != -1 && !remort && !( vamp ) )
    {
       sprintf( buf, "You are now level %d in your %s class.\n\r",
          value, class_table[class].class_name );
       send_to_char( buf, victim );
   ...
   {
      sprintf( buf, "You are now level %d in your %s class.\n\r",
         value, remort_table[class].class_name );
      send_to_char( buf, victim );
   ...
   if ( argument[0] == '\0' )
   {
      ...
      for ( a = 0; monitor_table[a].min_level != 0; a++ )
      {
         ...
         if ( IS_SET( ch->pcdata->monitor, monitor_table[a].channel ) )
            sprintf( buf, "@@W%s@@g", monitor_table[a].on_name );
         else
            sprintf( buf, "@@d%s@@g", monitor_table[a].off_name );
...
void do_otype( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
        send_to_char( "Otype <light scroll wand staff weapon beacon portal treasure armor potion clutch furniture trash trigger container quest drink_con key food money ", ch );
        send_to_char("stake boat corpse_npc corpse_pc fountain pill board soul piece matrix enchantment>\n\r", ch );
        return;
    }
...
(pObjIndex->item_type)-1 ].text ) )
            {
                found = TRUE;
                sprintf( buf, "<%s> [%5d] %s\n\r", tab_item_types[
(pObjIndex->item_type)-1 ].text, pObjIndex->vnum, pObjIndex->short_descr );
                safe_strcat(MAX_STRING_LENGTH, buf1, buf);
            }
        }
    }
...
void do_owear( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
        send_to_char( "Owear <take finger neck body head legs feet hands arms shield about waist wrist wield hold face ear clutch wield_2>\n\r", ch );
        return;
    }
    buf1[0] = '\0';
    fAll        = !str_cmp( arg, "all" );
...
bit_table_lookup(tab_wear_flags, pObjIndex->wear_flags) ) )
            {
                found = TRUE;
                sprintf( buf, "<%s> [%5d] %s\n\r",
bit_table_lookup(tab_wear_flags, pObjIndex->wear_flags), pObjIndex->vnum,
