...
int mana_cost( CHAR_DATA * ch, int sn)
{
 ...
 mincost=1000;
  ...
  if ( skill_table[sn].flag2 == WOLF )
  {
    ...
    if ( IS_WOLF( ch ) )
    {
      cost = UMAX( skill_table[sn].min_mana, ( ( skill_table[sn].min_mana * skill_lev ) / ( ch->pcdata->vamp_level ) ) );
    }
  }
  if ( IS_NPC( ch ) )
    mincost /= 2;
...
bool saves_spell( int level, CHAR_DATA *victim )
{
    ...
    save = 5 + ( get_psuedo_level( victim ) - level - victim->saving_throw );
...
*/
            ...
            && ( number_percent( ) < ( get_psuedo_level( victim ) - 70 ) )  )
    {
      ...
      act( "@@N$n's @@lc@@el@@ro@@ya@@ak@@N glows brightly as $N's spell hits it, and the spell is reflected@@N!!", victim, NULL, ch, TO_ROOM );
	...
	for ( vch = ch->in_room->first_person; vch; vch = vch_next )
	{
	    ...
	    {
            check_killer ( ch, victim );
		multi_hit( victim, ch, TYPE_UNDEFINED );  /* SRZ--fixed bad call--swap ch&v */
		break;
	    }
	}
        CUREF( vch_next );
...

{
    ...
    if ( obj==NULL )
    {
       act( "$n leans forward and lets fly with $s bad breath.  Eeeewwww!", ch, NULL, NULL, TO_ROOM ); 
       send_to_char( "Your breath takes it's affect!\n\r", ch );
    }
    else
    {
       act( "Green breath from $p strikes $n!", victim, obj, NULL, TO_ROOM );
       act( "Green breath from $p strikes you!", victim, obj, NULL, TO_CHAR );
    }
    return TRUE;
}
...
bool spell_call_lightning( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
	...
	if ( vch->in_room == ch->in_room )
	{
		...
		send_to_char( "A bolt of lightning strikes down onto you!", vch );      
...
bool spell_colour_spray( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    static const short dam_each[] = 
    {
...
bool spell_energy_drain( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
	...
	ch->hit         = UMAX( ch->max_hit, ch->hit + dam );
    }
...
bool spell_fireball( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
    ...
    {
       act( "A fireball shoots from $p and hits $n!", victim, obj, NULL, TO_ROOM );
       act( "A fireball shoots from $p and hits you!", victim, obj, NULL, TO_ROOM );
    }
    damage( ch, victim, dam, -1 );
    return TRUE;
}
...
bool spell_flamestrike( int sn, int level, CHAR_DATA *ch, void *vo, OBJ_DATA *obj )
{
    ...
    send_to_char( "A jet of flame roars down onto you!", victim );  
